...
func test_quest_bootstrap_smoke_hierarchy_repro_keeps_bootstrap_minimal_on_ui_pivot_hierarchy() -> void:
	var scene: PackedScene = load("res://scenes/repro/quest_bootstrap_smoke_hierarchy_repro.tscn")
	...
	assert_not_null(repro.get_node("XROrigin3D/UiPivot/QuestUiLayer/SubViewport/QuestPanel/Panel/Margin/Scroll/VBox/StatusLabel"))
