...
func _apply_curve(value: float, curve: String) -> float:
    ...
    match curve:
        "expo_soft":
            return signf(value) * (0.35 * abs_value * abs_value * abs_value + 0.65 * abs_value)
        "expo_hard":
            return signf(value) * (0.7 * abs_value * abs_value * abs_value + 0.3 * abs_value)
        _:
            return value
...
func _apply_output_processing(output_name: String, value: float, output_config: Dictionary) -> float:
    ...
    if expo > 0.0:
        abs_value = absf(value)
        value = signf(value) * (expo * abs_value * abs_value * abs_value + (1.0 - expo) * abs_value)
